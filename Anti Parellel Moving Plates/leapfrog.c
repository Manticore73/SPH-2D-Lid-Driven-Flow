...
static void lid_driven(int which, float barrier, 
                         float* x, float* v, float* vh, float v_imp);
...
static void reflect_bc(sim_state_t* s)
{
    ...
    for (int i = 0; i < n; ++i, x += 2, v += 2, vh += 2) {
        ...
        if (x[1] < YMIN) lid_driven(1, YMIN, x, v, vh, -1);
        if (x[1] > YMAX) lid_driven(1, YMAX, x, v, vh, 1);
    }
}
